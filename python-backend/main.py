...
#     model="groq/llama3-8b-8192", # Or "groq/llama3-70b-8192" if you prefer
#     name="Relevance Guardrail",
#     instructions=(
#         "Determine if the user's message is highly unrelated to a normal customer service "
#         "conversation with an airline (flights, bookings, baggage, check-in, flight status, policies, loyalty programs, etc.). "
...
def create_initial_context() -> AirlineAgentContext:
    """
    Factory for a new AirlineAgentContext.
    Provides static values for consistency instead of random generation.
    """
    ctx = AirlineAgentContext()
    ctx.account_number = "12345678"  # Static account number
    return ctx
...
async def on_seat_booking_handoff(context: RunContextWrapper[AirlineAgentContext]) -> None:
    """Set static flight number and confirmation number when handed off to the seat booking agent."""
    context.context.flight_number = "AA123"  # Static flight number
    context.context.confirmation_number = "ABC123"  # Static confirmation number

async def on_cancellation_handoff(context: RunContextWrapper[AirlineAgentContext]) -> None:
    """Set static confirmation and flight number when handing off to cancellation."""
    context.context.confirmation_number = "XYZ789"  # Static confirmation number
    context.context.flight_number = "BA456"  # Static flight number
...
triage_agent = Agent[AirlineAgentContext](
    ...
    instructions=(
        f"{RECOMMENDED_PROMPT_PREFIX} "
        "You are a helpful triaging agent. You can use your tools to delegate questions to other appropriate agents. "
        "When using a handoff tool (e.g., `transfer_to_seat_booking_agent`), include an empty 'parameters' object (e.g., `transfer_to_seat_booking_agent()` with `parameters: {}`) unless explicitly required by the tool's definition. "
        "The tools have no required parameters, so use an empty `parameters` object in the tool call."
    ),
    handoffs=[
        handoff(agent=flight_status_agent),
        handoff(agent=cancellation_agent, on_handoff=on_cancellation_handoff),
